...
def process_arch_dirs(b3dir, b4dir, b5dir):
    b3files = dict()
    b4files = dict()
    b5files = dict()
    mjds_set = set()
    ...
    for fname in glob.glob(f"{b4dir}/*"):
        #print(fname)
    ...
    for mjd in mjds_set:
        b345_files[mjd] = (b3files.get(mjd), b4files.get(mjd), b5files.get(mjd))
...
if __name__ == "__main__":
    ...
    parser.add_argument("b4templ", help="Band 4 Template")
    parser.add_argument("b5templ", help="Band 5 Template")
    parser.add_argument("b3n", help="No. of subbands for band 3", type=int)
    parser.add_argument("b4n", help="No. of subbands for band 4", type=int)
    ...
    parser.add_argument("b4_arch_dir", help="Folder containing band 4 PSRFITS archives")
    ...
    b4templfile = check_file(args.b4templ)
    ...
    b4dir = check_folder(args.b4_arch_dir)
    b5dir = check_folder(args.b5_arch_dir)
    
    print("Par file:", parfile)
    print("Template files:", b3templfile, ';', b4templfile, ';', b5templfile)
    print("Frequency subbands:", b3n_sub, b4n_sub, b5n_sub)
    print("Archive Directories:", b3dir, ';', b4dir, ';', b5dir)
    
    # Process archive directories
    b345_archive_files = process_arch_dirs(b3dir, b4dir, b5dir)
    
    print('{} epochs to process'.format(len(b345_archive_files.keys())))
    print('')
    ...
    # print b3 template info
    print("Template epoch is {}.".format(b3templ_epoch))
    print("Template has {} MHz bandwidth.".format(b3templ_bw))
    print("Template has {} channels.".format(b3templ_nchan))
    print("Fiducial DM present in the template is {}.".format(b3templ_dm))
    
    # Checking band 4 template
    b4templ_archive = psrchive.Archive_load(b4templfile)
    b4templ_bw = b4templ_archive.get_bandwidth()
    b4templ_nchan = b4templ_archive.get_nchan()
    b4templ_dm = b4templ_archive.get_dispersion_measure()
    b4templ_epoch = b4templ_archive.get_Integration(0).get_start_time().intday()
    
    # print b4 template info
    print("Template epoch is {}.".format(b4templ_epoch))
    print("Template has {} MHz bandwidth.".format(b4templ_bw))
    print("Template has {} channels.".format(b4templ_nchan))
    print("Fiducial DM present in the template is {}.".format(b4templ_dm))
    ...
    # print b5 template info
    print("Template epoch is {}.".format(b5templ_epoch))
    print("Template has {} MHz bandwidth.".format(b5templ_bw))
    print("Template has {} channels.".format(b5templ_nchan))
    ...
    # if b3templ_bw=200 MHz, then splitting the band 3 template into two for 100 MHz observations.
    if b3templ_bw==200:
    ...
    # if b4templ_bw=200 MHz, then splitting the band 4 template into two for 100 MHz observations.
    if b4templ_bw==200:
        split_b4nchan = b4templ_nchan/2
        os.system("psrsplit -c {} {}".format(split_b4nchan, b4templfile))
        # Pick the correct file from the two split files.
        b4templfile_100 = '.'.join(b4templfile.split('.')[:-1] + ["0001_0000"] + b4templfile.split('.')[-1:])
        b4templfile_100_other = '.'.join(b4templfile.split('.')[:-1] + ["0000_0000"] + b4templfile.split('.')[-1:])
        os.unlink(b4templfile_100_other)
    
        b4templfiles = { 100 : b4templfile_100,
                         200 : b4templfile      }

    # if b4templ_bw=100 MHz, then no need to split
    if b4templ_bw==100:
        split_b4nchan = b4templ_nchan
        b4templfiles = { 100 : b4templfile	}
                     
    # if b5templ_bw=200 MHz, then splitting the band 5 template into two for 100 MHz observations.
    if b5templ_bw==200:
    ...
    for mjd, (b3_arch_file, b4_arch_file, b5_arch_file) in b345_archive_files.items():
        print('')
        print("Processing", mjd, str(b3_arch_file), str(b4_arch_file), str(b5_arch_file))
        
        b3_arch_file_tmp = b4_arch_file_tmp = b5_arch_file_tmp = None
        ...
        if b4_arch_file is not None:
            b4_archive = psrchive.Archive_load(check_file(b4_arch_file))
            
            # Check 1
            b4_nsubints = b4_archive.get_nsubint()
            if b4_nsubints != 1:
                raise ValueError("The band 4 archive should be time scrunched. Use `$ pam -T` to do this.")
            
            # Check 2
            b4_bw = b4_archive.get_bandwidth()
            if b4_bw < 0:
                raise ValueError("The band 4 archive should have positive bandwidth. Use `$ pam --reverse_freq` to do this.")
            
            # Check 3
            b4_ra = b4_archive.get_coordinates().ra().getRadians()
            b4_dec = b4_archive.get_coordinates().dec().getRadians()
            if b4_ra==0 and b4_dec==0:
                raise ValueError("The archive coordinates should be corrected. Use `$ update_coords.sh` to do this.")
            
            # Check 4
            b4_freq = b4_archive.get_centre_frequency()
            if not (b4_freq>=550 and b4_freq<=750):
                raise ValueError("The archives are not of the correct Band(s). Check these files.")
            
            # Template selection
            b4_templfile_select = b4templfiles[int(b4_bw)]
            
            b4_arch_file_tmp = b4_arch_file + ".tmp"
            shutil.copyfile(b4_arch_file, b4_arch_file_tmp)
            ...
            # Frequency scrunching
            b4_nchan_new = int(b4_bw*split_b4nchan/100)
            os.system(f"pam --setnchn {b4_nchan_new} -m {b4_arch_file_tmp}")
        ...
        # Now run dmcalc.
        if b3_arch_file_tmp is not None and b4_arch_file_tmp is not None and b5_arch_file_tmp is not None:
          dmcalc_cmd = f"python3 /Data/prana/InPTA-DR/DR2/NB_results/scripts/DMCalc/DR2run2/dmcalc_dr2_run2_V1.py {b3_arch_file_tmp} {b4_arch_file_tmp} {b5_arch_file_tmp} -E {parfile} -M {b3_templfile_select} {b4_templfile_select} {b5_templfile_select} -b3n {b3n_sub} -b4n {b4n_sub} -b5n {b5n_sub} "
        elif b3_arch_file_tmp is not None and b4_arch_file_tmp is None and b5_arch_file_tmp is not None:
          dmcalc_cmd = f"python3 /Data/prana/InPTA-DR/DR2/NB_results/scripts/DMCalc/DR2run2/dmcalc_dr2_run2_V1.py {b3_arch_file_tmp} {b5_arch_file_tmp} -E {parfile} -M {b3_templfile_select} {b5_templfile_select} -b3n {b3n_sub} -b5n {b5n_sub} "
        elif b3_arch_file_tmp is not None and b4_arch_file_tmp is None and b5_arch_file_tmp is None:
          dmcalc_cmd = f"python3 /Data/prana/InPTA-DR/DR2/NB_results/scripts/DMCalc/DR2run2/dmcalc_dr2_run2_V1.py {b3_arch_file_tmp} -E {parfile} -M {b3_templfile_select} -nch {b3n_sub} "
        elif b3_arch_file_tmp is None and b4_arch_file_tmp is None and b5_arch_file_tmp is not None:
          dmcalc_cmd = f"python3 /Data/prana/InPTA-DR/DR2/NB_results/scripts/DMCalc/DR2run2/dmcalc_dr2_run2_V1.py {b5_arch_file_tmp} -E {parfile} -M {b5_templfile_select} -nch {b5n_sub} "
        ...
        if b4_arch_file_tmp is not None:
            os.unlink(b4_arch_file_tmp)
        if b5_arch_file_tmp is not None:
            os.unlink(b5_arch_file_tmp)
    
    os.unlink(b3templfile_100)
#    os.unlink(b4templfile_100)
#    os.unlink(b5templfile_100)
